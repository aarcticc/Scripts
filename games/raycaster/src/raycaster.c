/**
 * @file raycaster.c
 * @brief Core implementation of a simple 2D raycasting engine using SDL2.
 *
 * This file contains the main functions for initializing graphics, rendering frames
 * using raycasting, handling player movement and rotation, and cleaning up resources.
 * The raycasting algorithm simulates a 3D environment by casting rays from the player's
 * viewpoint into a 2D map and drawing vertical wall slices based on intersection distances.
 *
 * Functions:
 * - int init_graphics(Graphics *gfx): Initializes SDL2, creates window, renderer, texture, and pixel buffer.
 * - void shutdown_graphics(Graphics *gfx): Frees all SDL2 resources and pixel buffer.
 * - void render_frame(Graphics *gfx, Player *player, int map[8][8]): Renders a single frame using raycasting.
 * - void handle_input(Player *player, const Uint8 *keystate, int map[8][8]): Handles keyboard input for player movement and rotation.
 *
 * Dependencies:
 * - SDL2 library for graphics and input handling.
 * - raycaster.h for structure and constant definitions.
 *
 * Usage:
 * 1. Call init_graphics() to set up the graphics context.
 * 2. In the main loop, call handle_input() to process user input and render_frame() to draw the scene.
 * 3. Call shutdown_graphics() before exiting to free resources.
 */
